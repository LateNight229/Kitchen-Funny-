...
public class UIOrderManager : MonoBehaviour
{
    ...
    [SerializeField] private float timeShowVisual = 0.5f;
    private void Start()
    {
        
    }
    public void HandleSubmitDishFailed()
    {
        for (int i = 0; i < dishPrefabs.Count; i++)
        {
            var visualUIOrder = dishPrefabs[i].GetComponent<VisualUIOrder>();
            if(visualUIOrder != null)
            {
                visualUIOrder.HandleVisualUIOrder(false, timeShowVisual);
            }
        }
    }
    ...
    private void HandleDishOrdersUI(TypeDishLevel1 currentDishsRequired, bool value)
    {
        ...
        else
        {
            ...
            if (numberClear >= 0)
            {
                PreparePrefabForPool(prefab: dishPrefabs[numberClear], objPool);
    ...
    private void SpawnDishUI( ObjectPool objPool, TypeDishLevel1 currentDishsRequired)
    {
        ...
        if ( dishOrderUIPrfab != null )
        {
           // dishOrderUIPrfab.transform.parent = PanelUiOrderDish.transform;
            var reactDishOrderUIPrfab = dishOrderUIPrfab.GetComponent<RectTransform>();
            if(reactDishOrderUIPrfab != null) { reactDishOrderUIPrfab.SetParent(PanelUiOrderDish.transform, false); }
            dishOrderUIPrfab.transform.localScale = Vector3.one;
            dishOrderUIPrfab.transform.localRotation = Quaternion.identity;
            dishOrderUIPrfab.transform.localPosition = Vector3.one;
    ...
    private void PreparePrefabForPool(GameObject prefab, ObjectPool objPool)
    {
        ...
        var visualUIOrder = prefab.GetComponent<VisualUIOrder>();
        if (visualUIOrder != null) visualUIOrder.HandleVisualUIOrder(true, timeShowVisual);

        StartCoroutine(ReturnToPool(prefab, objPool, timeShowVisual));
    }
    private IEnumerator ReturnToPool(GameObject prefab, ObjectPool objPool, float delay)
    {
